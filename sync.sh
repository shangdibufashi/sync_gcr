...
git_init(){
    git config --global user.name "jiapinai"
    git config --global user.email develop@jiapinai.com
    git remote rm origin
    git remote add origin git@github.com:jiapinai/gcr_sync.git
    git pull
...
process_images(){
    ...
    echo 'image_fetch done'
    git status
    COMMIT_FILES_COUNT=$(git status -s|wc -l)
    TODAY=$(date +%F)
    echo "COMMIT_FILES_COUNT=$COMMIT_FILES_COUNT"
    if [ $COMMIT_FILES_COUNT -ne 0 ];then
        git add -A
        git commit -m "Synchronizing completion at $TODAY"
        git push -u origin develop
    fi
    echo 'image_process start'
